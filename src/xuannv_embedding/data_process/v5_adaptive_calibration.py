"""Calibrate candidate window placement on independently frozen training textures."""
...
def evaluate_texture(reference, valid, *, gsd):
    ...
    for mask_name, mask in [
        ...
        for delta in CALIBRATION["shifts_yx_pixels"]:
            ...
            candidate = audit_adaptive(reference, moving, mask, moved, gsd=gsd)
    ...
    controls = [
        {"name": name, "measurement": audit_adaptive(a, b, mask, mask, gsd=gsd)}
...
def calibrate_adaptive(
    ...
    implementation = {
        ...
        **{
            ...
            for name in ["v5_adaptive_alignment.py", "v5_adaptive_calibration.py"]
        },
    }
    fingerprint = {
        "family": family,
    ...
    root = (
        dataset_root / "quality/alignment/adaptive_calibration" / family / _digest(fingerprint)[:20]
    )
    ...
    progress = report_root / f"adaptive_calibration_{family}.json"
