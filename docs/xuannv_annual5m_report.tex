...
三分支使用教师--学生的潜变量预测作为主要自监督目标：
学生只看到随机留出的观测子集，教师由同一模型的指数滑动平均参数产生目标嵌入；教师输入包含另一观测子集，
...
M2和M3构成正式训练主线，M4作为后续实验。每组至少报告四类指标：低分-only、5m-only、PAN-only和完整输入；
同时报告有高分与无高分、云覆盖高与低、相对配准误差小与大的空间块。若高分只提高重建损失而不能提高冻结嵌入下游结果，
不进入正式模型。若M2导致教师滞后、表示坍塌或跨年度不稳定，则退回M1并增加来源归一化和目标遮挡比例消融。
...
v1.5 & 2026-09-20 & 固定年度训练目标、低分锚点残差门控、观测质量条件和算法消融顺序。\\\bottomrule
\end{tabularx}
\end{document}
